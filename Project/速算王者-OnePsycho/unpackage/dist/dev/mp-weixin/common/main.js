...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _home_vue_vue_type_template_id_367c7ccc_name_components___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./home.vue?vue&type=template&id=367c7ccc&name=components& */ 13);
...
var _listTwoReduce = _interopRequireDefault(__webpack_require__(/*! ../../utils/list-two-reduce.js */ 17));
var _listTwoAdd = _interopRequireDefault(__webpack_require__(/*! ../../utils/list-two-add.js */ 18));
var _listTwoRide = _interopRequireDefault(__webpack_require__(/*! ../../utils/list-two-ride.js */ 19));
var _listThreeAdd = _interopRequireDefault(__webpack_require__(/*! ../../utils/list-three-add.js */ 20));function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} //
//
//
//
//
...
//
//
var _default = { data: function data() {return { isChallenSuc: false, numsList: [0, 1, 2, 3, 4, 5, 6, 7, 8], curAnswer: '', curIndex: 0, curProgress: 0, numA: null, numB: null, count: 10, nums: null, type: null, min: 0, minStr: '', second: 0, secStr: '', ms: 0, msStr: '', timer: null, isShowModal: false, questionList: [], targetTime: '' };}, onLoad: function onLoad(options) {// 判断是否为挑战模式
    if (!options.list) {this.type = options.type;this.count = options.count;this.init();} else {this.count = 10;this.type = options.type;this.targetTime = options.time;this.questionList = JSON.parse(options.list);this.numA = this.questionList[0].a;this.numB = this.questionList[0].b;clearInterval(this.timer);this.timer = setInterval(this.show, 10);}}, /* 分享给好友 */onShareAppMessage: function onShareAppMessage(res) {if (res.from === 'button') {var typeStr = this.type === 'twoAdd' ? '两位数加法' : this.type === 'twoReduce' ? '两位数减法' : this.type === 'twoRide' ? '两位数乘法' : '三位数加法';return { title: this.count + '道' + typeStr + '速算我只用了 ' + this.minStr + ':' + this.secStr + ':' + this.msStr + ' 就完成了，你呢？', imageUrl: 'https://psycho-1300960840.cos.ap-chengdu.myqcloud.com/bg_share.png', path: '/pages/welcome/home?list=' + JSON.stringify(this.questionList) + '&time=' + this.minStr + ':' + this.secStr + ':' + this.msStr + '&type=' + this.type };} else if (res.from === 'menu') {return { title: '速算王者-你速算有我厉害吗?', imageUrl: 'https://psycho-1300960840.cos.ap-chengdu.myqcloud.com/bg_share.png', path: '/pages/welcome/home' };}}, methods: { /* 初始化 */init: function init() {var list = this.type === 'twoAdd' ? _listTwoAdd.default : this.type === 'twoReduce' ? _listTwoReduce.default : this.type ===
      'twoRide' ? _listTwoRide.default : _listThreeAdd.default;
    ...
    onItemClick: function onItemClick(val) {
      if (val === 'cancel') {
        ...
        if (this.curIndex < this.count - 1) {
          ...
          /* 检测是挑战成功还是挑战失败 */
          if (this.targetTime) {
            var arr = this.targetTime.split(':').map(function (i) {return Number(i);});
            var targetCount = arr[0] * 6000 + arr[1] * 100 + arr[2];
            var myCount = this.min * 6000 + this.second * 100 + this.ms;
